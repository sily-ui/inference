    ...
    def generate_config(
        ...
        if enable_twitter or enable_reddit:
            ...
            platform_configs = self._generate_platform_configs(
                ...
                num_agents=len(all_agent_configs),
            ...
            if twitter_config:
                reasoning_parts.append(
                    f"Twitter权重: recency={twitter_config.recency_weight}, popularity={twitter_config.popularity_weight}, relevance={twitter_config.relevance_weight}"
                )
            if reddit_config:
                reasoning_parts.append(
                    f"Reddit权重: recency={reddit_config.recency_weight}, popularity={reddit_config.popularity_weight}, relevance={reddit_config.relevance_weight}"
                )
...
{{
    ...
    def _generate_platform_configs(
        ...
        enable_twitter: bool = True,
        enable_reddit: bool = True,
    ) -> Dict[str, Optional[PlatformConfig]]:
        """
        由LLM根据模拟需求智能生成平台推荐算法权重

        权重设计原则:
        - recency_weight: 时间新鲜度权重，影响最新内容的曝光
        - popularity_weight: 热度权重，影响热门内容的传播
        - relevance_weight: 相关性权重，影响与用户兴趣匹配的内容
        - viral_threshold: 病毒传播阈值，达到后快速扩散
        - echo_chamber_strength: 回声室效应，相似观点聚集程度
        """

        entity_type_str = ", ".join(entity_types[:10]) if entity_types else "未知"
        topics_str = ", ".join(hot_topics) if hot_topics else "无特定话题"

        prompt = f"""根据以下模拟场景，为社交媒体平台生成推荐算法权重配置。
...
## 任务
请根据场景特征，为每个平台生成合适的推荐算法参数。

### 权重设计原则：
- **recency_weight (0.1-0.6)**: 时间新鲜度
  - 突发事件、舆情分析 → 偏高 (0.4-0.6)，让最新信息快速传播
  - 长期趋势分析、学术讨论 → 偏低 (0.1-0.3)，让优质内容持续曝光
  
- **popularity_weight (0.1-0.6)**: 热度权重
  - 热点事件、明星话题 → 偏高 (0.4-0.6)，利用从众心理
  - 小众兴趣、专业讨论 → 偏低 (0.1-0.3)
  
- **relevance_weight (0.1-0.5)**: 相关性权重
  - 个性化推荐场景 → 偏高 (0.3-0.5)
  - 公共话题讨论 → 偏低 (0.1-0.3)
  
- **viral_threshold (5-50)**: 病毒传播阈值
  - Agent规模大、热点事件 → 偏低 (5-15)，容易触发病毒式传播
  - Agent规模小、理性讨论 → 偏高 (20-50)
  
- **echo_chamber_strength (0.1-0.9)**: 回声室效应
  - 政治敏感话题、立场对立 → 偏高 (0.6-0.9)，强化立场
  - 中性讨论、知识分享 → 偏低 (0.1-0.4)
...
如果某个平台未启用，返回 null。例如: {{"twitter": {{...}}, "reddit": null}}"""

        system_prompt = "你是社交媒体推荐算法专家。返回纯JSON格式，权重为浮点数。"
        ...
        try:
            ...
            if enable_twitter and result.get("twitter"):
                t = result["twitter"]
                configs["twitter"] = PlatformConfig(
                    platform="twitter",
                    recency_weight=float(t.get("recency_weight", 0.4)),
                    popularity_weight=float(t.get("popularity_weight", 0.3)),
                    relevance_weight=float(t.get("relevance_weight", 0.3)),
                    viral_threshold=int(t.get("viral_threshold", 10)),
                    echo_chamber_strength=float(t.get("echo_chamber_strength", 0.5)),
                )
            else:
                configs["twitter"] = None

            if enable_reddit and result.get("reddit"):
                r = result["reddit"]
                configs["reddit"] = PlatformConfig(
                    platform="reddit",
                    recency_weight=float(r.get("recency_weight", 0.3)),
                    popularity_weight=float(r.get("popularity_weight", 0.4)),
                    relevance_weight=float(r.get("relevance_weight", 0.3)),
                    viral_threshold=int(r.get("viral_threshold", 15)),
                    echo_chamber_strength=float(r.get("echo_chamber_strength", 0.6)),
                )
            else:
                configs["reddit"] = None

            logger.info(
                f"LLM生成的平台配置: {result.get('twitter', {}).get('reasoning', '')}, {result.get('reddit', {}).get('reasoning', '')}"
            )
            return configs
            ...
            if enable_twitter:
                configs["twitter"] = PlatformConfig(
                    ...
                    viral_threshold=max(5, num_agents // 10),
            ...
            if enable_reddit:
                configs["reddit"] = PlatformConfig(
                    ...
                    relevance_weight=0.3,
                    viral_threshold=max(10, num_agents // 8),
