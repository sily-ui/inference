...
from ..models.database import (
    db,
    Report as DbReport,
    ReportSection as DbReportSection,
    ReportProgress as DbReportProgress,
)
...
1. 具有洞察力和悬念感，引发读者好奇心
2. 使用有力的动词和具象化的表达
3. 体现预测性和前瞻性（如"未来""趋势""演变""图景"等）
4. 避免平淡无奇的描述性标题
5. 可以尝试使用主副标题形式增强层次感

好的标题示例：
- "热潮之后：XX事件对行业格局的重塑与分化"
- "从喧嚣到沉淀：XX现象背后的长期趋势研判"
- "破局与重构：XX变量如何改写未来竞争规则"
- "涟漪效应：XX事件的跨领域影响深度解析"

【章节标题要求】
每个章节标题应该：
1. 简洁有力，3-8个字为宜
2. 使用动词或具象化表达，避免抽象名词堆砌
3. 体现该章节的核心洞察点
4. 章节之间要有逻辑递进关系
...
{
    "title": "报告主标题",
    "summary": "报告摘要（一句话概括核心预测发现，要有洞察力和冲击力）",
    "sections": [
        {
            "title": "章节标题（简洁有力）",
            "description": "章节内容描述"
...
【分析要有深度】
- 不要停留在表面描述，要揭示背后的原因和机制
- 使用"这意味着...""这表明...""这反映出..."等句式深化分析
- 将具体现象与宏观趋势联系起来

【表达要有力量】
- 使用主动语态和有力的动词
- 避免模糊词汇（"可能""大概""似乎"），用数据说话
- 适当使用修辞手法增强说服力（对比、排比、设问等）
...
class ReportManager:
    ...
    def save_outline(cls, report_id: str, outline: ReportOutline) -> None:
        ...
        try:
            ...
            if not db_report:
                db_report = DbReport(id=report_id)
                db.session.add(db_report)
            db_report.outline = outline.to_dict()
            db_report.title = outline.title
            db_report.summary = outline.summary
            db.session.commit()
        except Exception as e:
            logger.warning(f"保存大纲到数据库失败: {e}")
            db.session.rollback()
    ...
    def save_section(
        ...
        # 保存到数据库
        try:
            db_section = DbReportSection.query.filter_by(
                report_id=report_id, section_index=section_index
            ).first()
            if not db_section:
                db_section = DbReportSection(
                    report_id=report_id, section_index=section_index
                )
                db.session.add(db_section)
            db_section.title = section.title
            db_section.content = cleaned_content
            db.session.commit()
        except Exception as e:
            logger.warning(f"保存章节到数据库失败: {e}")
            db.session.rollback()
    ...
    def get_generated_sections(cls, report_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            db_sections = (
                DbReportSection.query.filter_by(report_id=report_id)
                .order_by(DbReportSection.section_index)
                .all()
            )
            if db_sections:
                return [
                    {
                        "filename": f"section_{s.section_index:02d}.md",
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        try:
            ...
            if db_report:
                outline = None
                if db_report.outline:
                    outline_data = db_report.outline
                    sections = []
                ...
                return Report(
                    ...
                    simulation_id=db_report.simulation_id or "",
                    graph_id="",
                    ...
                    outline=outline,
                    markdown_content=db_report.markdown_content or "",
                    created_at=db_report.created_at.isoformat()
                    if db_report.created_at
                    else "",
                    completed_at=db_report.updated_at.isoformat()
                    if db_report.updated_at
                    else "",
                    error=db_report.error,
                )
        except Exception as e:
            logger.warning(f"从数据库读取报告失败: {e}")
    ...
    def get_report_by_simulation(cls, simulation_id: str) -> Optional[Report]:
        ...
        # 优先从数据库读取
        try:
            db_report = DbReport.query.filter_by(simulation_id=simulation_id).first()
            if db_report:
                return cls.get_report(db_report.id)
        except Exception as e:
            logger.warning(f"从数据库查询报告失败: {e}")
    ...
    def list_reports(
        ...
        try:
            ...
            db_reports = query.order_by(DbReport.created_at.desc()).limit(limit).all()
            if db_reports:
                reports = [cls.get_report(r.id) for r in db_reports]
                return [r for r in reports if r]
        except Exception as e:
            logger.warning(f"从数据库列出报告失败: {e}")
    ...
    def delete_report(cls, report_id: str) -> bool:
        ...
        try:
            db_report = DbReport.query.get(report_id)
            if db_report:
                db.session.delete(db_report)
                db.session.commit()
                logger.info(f"报告已从数据库删除: {report_id}")
        except Exception as e:
            logger.warning(f"从数据库删除报告失败: {e}")
            db.session.rollback()
