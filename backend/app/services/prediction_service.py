...
请直接输出结论文字，不要有任何格式标记。"""
    ...
    def simulate_intervention(
        ...
        prompt = f"""你是一位资深的舆情危机公关顾问，擅长用通俗易懂的语言分析干预策略的效果。请模拟以下干预策略的效果。
...
3. 每个字段的具体要求：
   - expected_effect字段：像跟朋友聊天一样说这个策略怎么样。❌错误示例："降温效果显著，能有效遏制舆情扩散，情绪改善明显" ✅正确示例："这招挺管用的，能让热度降个15%左右，大家情绪会好一些，但可能显得有点生硬"
   - risk字段：说人话的风险提示。❌错误示例："可能引发次生舆情风险" ✅正确示例："说错话可能火上浇油，越描越黑"

请分析这个干预策略的效果，要求：
1. expected_effect字段：用大白话描述效果，像跟朋友聊天一样
2. heat_change：预期热度变化（百分比，正数表示上升，负数表示下降）
3. sentiment_change：预期情绪变化（正数表示变正面，负数表示变负面）
4. risk：说人话的风险提示
5. recommendation：推荐度（1-5星）

请以JSON格式返回：
{{
    "strategy": "{intervention}",
    "expected_effect": "用大白话描述效果，像跟朋友聊天",
    "heat_change": 10,
