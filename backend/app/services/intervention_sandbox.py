...
3. 每个字段的具体要求（必须严格执行）：
   - analysis字段（50-80字）：像跟朋友聊天一样分析这个策略怎么样，**必须紧密结合{event_summary}的具体背景，必须提到事件中的具体元素（比如产品名、事件核心争议点、涉及的人群等），绝对不能泛泛而谈。** ❌错误示例："这招对这个舆论事件挺管用的，能快速把热度压下去，但显得有点生硬，大家会觉得你在敷衍。" ✅正确示例："这招对iPhone18屏幕地震这事儿挺管用，能快速压热度，但大家会觉得你是在捂盖子，反而更不信任你。"
   - pros：说人话的优点列表，每个优点要具体可落地，要结合当前事件的特点。❌错误示例：["见效快，能快速稳住局面", "能给大家一个交代"] ✅正确示例：["能快速把灵动岛相关的不实讨论压下去", "能给激动的果粉一个明确的信号"]
   - cons：说人话的缺点列表，每个缺点要具体实际，要和当前事件的情况匹配。❌错误示例：["显得有点生硬", "可能错过跟用户解释清楚的机会"] ✅正确示例：["容易被骂做贼心虚，觉得你肯定有问题才不敢说细节", "反而会让大家更在意屏幕质量的问题"]
   - recommendation（60-100字）：直接说结论，别绕弯子，要给出具体的执行建议，**必须结合当前事件的具体情况。** ❌错误示例："建议主要用策略4，一个一个回应网友的质疑，前期可以先发个官方声明稳住局面。" ✅正确示例："建议主要用策略4，针对大家吐槽的灵动岛和屏幕质量问题一个个回应，前期可以先发个简短声明稳住果粉情绪，争取准备时间。"
