...
from ..utils.llm_client import get_token_stats
...
@report_bp.route("/token-stats", methods=["GET"])
def get_token_stats_api():
    ...
    """
    try:
        stats = get_token_stats().get_stats()

        # 计算预估费用 (MiniMax-M2.5)
        cached_price = 0.000210  # ¥/K
        input_price = 0.002100  # ¥/K
        output_price = 0.008400  # ¥/K

        prompt_excl_cached = max(0, stats["prompt_tokens"] - stats["cached_tokens"])
        estimated_cost = (
            stats["cached_tokens"] / 1000 * cached_price
            + prompt_excl_cached / 1000 * input_price
            + stats["completion_tokens"] / 1000 * output_price
        )

        return jsonify(
            {
                "success": True,
                "data": {**stats, "estimated_cost": round(estimated_cost, 4)},
            }
        )

    except Exception as e:
        logger.error(f"获取Token统计失败: {str(e)}")
        return jsonify({"success": False, "error": str(e)}), 500


@report_bp.route("/token-stats/reset", methods=["POST"])
