    ...
    <div class="scroll-container">
      ...
      <div v-if="phase >= 4" class="step-card active">
        ...
        <div class="card-content">
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">系统自动规划推演现实 <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> 小时，每轮代表现实 <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> 分钟时间流逝</span>
              </div>
          ...
          <!-- 资源消耗预估 -->
          <div v-if="phase >= 4 && tokenEstimate.agentCount > 0" class="resource-estimate-section">
            <div class="estimate-header">
              <span class="section-title">资源消耗预估</span>
              <span class="section-desc">基于当前配置的资源消耗估算</span>
            </div>
            ...
            <div class="estimate-cards">
              <div class="estimate-card">
                ...
                <div class="estimate-info">
                  <span class="estimate-value">{{ tokenEstimate.totalTokens.toLocaleString() }}</span>
                  <span class="estimate-label">预估Token消耗</span>
                </div>
              </div>
              
              <div class="estimate-card">
                <div class="estimate-icon cost">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <div class="estimate-info">
                  <span class="estimate-value">¥{{ tokenEstimate.estimatedCost }}</span>
                  <span class="estimate-label">预估费用 (约)</span>
                </div>
              </div>
              
              <div class="estimate-card">
                <div class="estimate-icon time">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <circle cx="12" cy="12" r="10"></circle>
                    <polyline points="12 6 12 12 16 14"></polyline>
                  </svg>
                ...
                <div class="estimate-info">
                  <span class="estimate-value">{{ runtimeEstimate.display }}</span>
                  <span class="estimate-label">预估运行时间</span>
                </div>
              </div>
            </div>
            
            <div class="estimate-detail">
              <span class="detail-item">Agent: {{ tokenEstimate.agentCount }} × 轮数: {{ tokenEstimate.rounds }} × 平台: {{ tokenEstimate.platformCount }}</span>
            </div>
          </div>
...
const tokenEstimate = computed(() => {
  ...
  const baseTokensPerAgentPerRound = 800
  const totalTokens = Math.round(agentCount * rounds * baseTokensPerAgentPerRound * platformCount * 0.5)
  
  // MiniMax-M2.5 价格 (¥/K Tokens)
  const cachedInputPrice = 0.000210    // 缓存输入
  const inputPrice = 0.002100           // 输入
  const outputPrice = 0.008400          // 输出
  
  // 假设 60% 输入(其中50%缓存), 40% 输出
  const cachedTokens = Math.round(totalTokens * 0.6 * 0.5)
  const inputTokens = Math.round(totalTokens * 0.6 * 0.5)
  const outputTokens = Math.round(totalTokens * 0.4)
  
  const cachedCost = cachedTokens / 1000 * cachedInputPrice
  const inputCost = inputTokens / 1000 * inputPrice
  const outputCost = outputTokens / 1000 * outputPrice
  const totalCost = cachedCost + inputCost + outputCost
...
const runtimeEstimate = computed(() => {
  ...
  // 基准: 100个Agent运行40轮约需 25-30 分钟
  // 时间与Agent数量和轮数成正比
  const baseMinutes = 27 // 100 agents * 40 rounds
  const ratio = (agentCount * rounds) / (100 * 40)
  const estimatedMinutes = Math.round(baseMinutes * ratio)
  
  if (estimatedMinutes < 60) {
    return {
      minutes: estimatedMinutes,
      display: `约 ${estimatedMinutes} 分钟`
    }
  } else {
...
.estimate-header .section-title {
  font-size: 14px;
  font-weight: 600;
  color: #1E293B;
}

.estimate-header .section-desc {
  font-size: 12px;
  color: #64748B;
}
