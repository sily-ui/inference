<template>
  <div class="process-page">
    ...
    <nav class="navbar">
      ...
      <!-- 步骤进度条 -->
      <div class="nav-center step-progress-bar">
        <div 
...
const goToStep = (idx) => {
  // 只能跳转到已完成或当前步骤
  const currentIdx = currentPhase.value >= 0 ? currentPhase.value + 1 : 0
  ...
  if (idx < currentIdx) {
    // 标记当前步骤需要重新运行
    steps.value[currentIdx].needRerun = true
    // 设置为查看模式
    steps.value[idx].status = 'viewing'
    // 更新 currentPhase 为查看的步骤
    currentPhase.value = idx - 1
  }
}
...
const statusText = computed(() => {
  const currentIdx = currentPhase.value >= 0 ? currentPhase.value + 1 : 0
  if (currentIdx >= 5) return '全部完成'
  if (error.value) return '构建失败'
  return `步骤 ${currentIdx + 1}/5: ${steps.value[currentIdx]?.name || ''}`
})
